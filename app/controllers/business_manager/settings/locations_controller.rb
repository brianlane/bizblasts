module BusinessManager
  module Settings
    class LocationsController < ApplicationController
      ...
      def index
        @locations = policy_scope([:business_manager, :settings, @business.locations])
      end
      ...
      def update
        ...
        if @location.update(processed_params)
          ...
          if @location == @business.default_location && params[:sync_to_business] == '1'
            sync_with_business
      ...
      def sync_with_business
        ...
        Rails.logger.info "[LOCATIONS] Synced default location ##{@location.id} info to business ##{@business.id}"
      end
      
      # Ensure that every business has at least one default location
      def ensure_default_location
        return if @business.locations.exists?
        ...
        hours = @business.hours.present? ? @business.hours : default_hours
        
        default_location = @business.locations.create!(
          name: "Main Location",
        ...
        Rails.logger.info "[LOCATIONS] Created default location ##{default_location.id} for business ##{@business.id}"
      rescue => e
