...
class DomainMailer < ApplicationMailer
  ...
  def assign_dns_instructions!
    if defined?(DomainProvider) && DomainProvider.caddy?
      @dns_provider          = 'caddy'
      @dns_apex_target_ip    = bizblasts_public_ip
      @dns_www_target_type   = 'A'
      @dns_www_target_value  = bizblasts_public_ip
  ...
  def bizblasts_public_ip
    ENV['BIZBLASTS_PUBLIC_IP'].presence || begin
      require 'resolv'
      Resolv::DNS.open do |r|
        recs = r.getresources('bizblasts.com', Resolv::DNS::Resource::IN::A)
        recs.first&.address&.to_s
      end
    rescue StandardError
      nil
    end || '<your BizBlasts server IP — contact support>'
  end
end