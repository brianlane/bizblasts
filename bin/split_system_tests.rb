...
# Script to automatically split system tests into 8 balanced groups
require 'find'
...
def split_tests_evenly(tests, num_groups = 8)
...
def main
  if ARGV[0] == '--help' || ARGV[0] == '-h'
    ...
    puts "  group_number: 1-8 (returns tests for that group)"
  ...
  groups = split_tests_evenly(tests, 8)
  
  if ARGV[0]
    group_num = ARGV[0].to_i
    if group_num >= 1 && group_num <= 8
      puts groups[group_num - 1].join(' ')
    else
      puts "Invalid group number. Use 1-8"
      exit 1
    end
  else
