...
RSpec.describe AuthToken, type: :model do
  ...
  describe '.consume!' do
    ...
    it 'successfully consumes a valid token' do
      consumed_token = AuthToken.consume!(valid_token.token, ip_address, user_agent)
    ...
    it 'returns nil for non-existent token' do
      result = AuthToken.consume!('nonexistent', ip_address, user_agent)
      expect(result).to be_nil
    end

    it 'returns nil for already used token' do
      valid_token.update!(used: true)
      result = AuthToken.consume!(valid_token.token, ip_address, user_agent)
      expect(result).to be_nil
    end
    ...
    it 'returns nil for IP address mismatch' do
      result = AuthToken.consume!(valid_token.token, '192.168.1.2', user_agent)
      expect(result).to be_nil
    end
    ...
    it 'succeeds despite user agent mismatch (logs warning)' do
      ...
      result = AuthToken.consume!(valid_token.token, ip_address, 'Different Browser')
      expect(result).to eq(valid_token)
      expect(result.used?).to be_truthy
    end

    it 'handles blank token gracefully' do
      expect(AuthToken.consume!('', ip_address, user_agent)).to be_nil
      expect(AuthToken.consume!(nil, ip_address, user_agent)).to be_nil
    end
  end
