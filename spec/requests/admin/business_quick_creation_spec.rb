...
RSpec.describe 'Admin Business Quick Creation', type: :request do
  ...
  describe 'POST /admin/businesses' do
    context 'with Free tier (subdomain)' do
    ...
    context 'with Standard tier (subdomain)' do
      let(:valid_attributes) do
        {
    ...
    context 'with Premium tier (custom domain)' do
      let(:valid_attributes) do
        {
  ...
  describe 'GET /admin/businesses/new' do
    ...
    it 'includes JavaScript for dynamic field switching' do
      get new_admin_business_path
      expect(response.body).to include('updateFieldsForTier')
      expect(response.body).to include('updateSlugFields')
      expect(response.body).to include('slugify')
    end

    it 'includes tier radio buttons with pricing' do
      get new_admin_business_path
      expect(response.body).to include('Free - $0/month')
      expect(response.body).to include('Standard - $9.99/month')
      expect(response.body).to include('Premium - $29.99/month')
    end
