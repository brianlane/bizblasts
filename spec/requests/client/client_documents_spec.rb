...
RSpec.describe "Client::Documents", type: :request do
  ...
  describe "GET /my-documents/:id" do
    ...
    context "when the client has no associated tenant customers" do
      let(:tenant_customer) { create(:tenant_customer, business: business, email: "other-#{client.email}") }

      it "redirects to the dashboard without raising a double render error" do
        expect { get client_document_path(document) }.not_to raise_error
