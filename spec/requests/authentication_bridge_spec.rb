  ...
  describe 'GET /auth/bridge' do
    context 'when user is signed in' do
      ...
      it 'creates a token and redirects to target domain with consumption URL' do
        get '/auth/bridge', params: { target_url: target_url, business_id: business.id }

        expect(response).to have_http_status(:redirect)
        
        # Should redirect to the consumption endpoint on target domain
        expect(response.location).to match(%r{^https://example\.com/auth/consume\?token=})
      ...
      it 'includes original query parameters in target URL' do
        ...
        get '/auth/bridge', params: { target_url: target_with_params, business_id: business.id }
      ...
      it 'handles URLs with custom ports' do
        ...
        get '/auth/bridge', params: { target_url: target_with_port, business_id: business.id }
      ...
      it 'rejects invalid target URLs' do
        get '/auth/bridge', params: { target_url: 'javascript:alert(1)' }
      ...
      it 'rejects non-HTTPS URLs in production' do
        ...
        get '/auth/bridge', params: { target_url: 'http://example.com/dashboard' }
        
        expect(response).to have_http_status(:bad_request)
        expect(response.body).to include('HTTPS required')
      end

      it 'allows HTTP URLs in development' do
        allow(Rails.env).to receive(:development?).and_return(true)
        
        get '/auth/bridge', params: { target_url: 'http://localhost:3000/dashboard' }
    ...
    context 'with missing target_url' do
      ...
      it 'returns bad request' do
        ...
        expect(response).to have_http_status(:bad_request)
        expect(response.body).to include('Target URL is required')
      end
    end
  end
  ...
  describe 'GET /auth/consume' do
    ...
    context 'with valid token' do
      ...
      it 'handles target URLs with query parameters' do
        ...
        get '/auth/consume',
            params: { token: token_with_query.token },
            headers: { 'REMOTE_ADDR' => '192.168.1.1', 'HTTP_USER_AGENT' => 'Test Browser' }
    ...
    context 'with invalid token' do
      it 'redirects to root with error for non-existent token' do
        get '/auth/consume', params: { token: 'nonexistent' }
