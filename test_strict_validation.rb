# Test the strict validation logic - no downgrade preservation
class TestBusiness
...
# Test scenarios
puts 'Testing STRICT validation logic (no downgrade preservation):'
...
# Test 3: Changes to non-premium (should all be blocked now)
puts "\n3. CHANGES TO NON-PREMIUM (should all be blocked):"
...
puts "   Premium→Free + Custom Domain: #{b7.errors.empty? ? 'FAIL (should be blocked)' : 'PASS (correctly blocked)'}"

b8 = TestBusiness.new('premium', 'custom_domain')
b8.tier = 'standard'
b8.custom_domain_requires_premium_tier
puts "   Premium→Standard + Custom Domain: #{b8.errors.empty? ? 'FAIL (should be blocked)' : 'PASS (correctly blocked)'}"
...
puts "\nSUMMARY - STRICT RULE:"
puts "- ONLY premium tier can use custom domains"
puts "- NO exceptions for downgrades - custom domains must be removed when downgrading"
puts "- All non-premium tier + custom domain combinations are blocked"
